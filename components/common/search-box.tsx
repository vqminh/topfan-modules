...
const SearchBox = React.forwardRef<HTMLInputElement, SearchProps>(
	({ className, onSubmit, onClear, ...rest }, ref) => {
		...
		return (
			<form
				className="relative pe-12 md:pe-14 bg-secondary overflow-hidden rounded-md w-full"
				noValidate
				role="search"
				onSubmit={onSubmit}
			>
				<label htmlFor="search" className="flex items-center py-0.5">
					...
					<input
						id="search"
						className="text-heading outline-none w-full h-12 lg:h-14 placeholder-gray-400 bg-secondary text-sm lg:text-base"
